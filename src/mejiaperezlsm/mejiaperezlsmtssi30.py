...
from ..core import skeleton_graph
...
_DESCRIPTION = """
MejiaPerezLsmTssi30 is a 30-label video dataset for Isolated Mexican Sign Language (LSM) recognition.
It contains 3,000 poses each one performed 25 times by four different people at different speeds, starting and ending times.
...
class MejiaPerezLsmTssi30(tfds.core.GeneratorBasedBuilder):
    """DatasetBuilder for mejiaperezlsmtssi30 dataset."""
    ...
    INFO = pd.read_csv(
        f"{current_path}/train_validation_info30.csv",
        index_col=0
    )
    DATA_URL = "https://storage.googleapis.com/slr_datasets_raw/MejiaPerezLSM30.zip"
    TSSI_ORDER = skeleton_graph.tssi_mejiaperez()[1]
    SEQUENCE_LENGTH = 20
    ...
    def _info(self) -> tfds.core.DatasetInfo:
        ...
        return tfds.core.DatasetInfo(
            ...
            features=tfds.features.FeaturesDict({
                ...
                'data': tfds.features.Tensor(shape=(self.SEQUENCE_LENGTH, len(self.TSSI_ORDER), 3), dtype=np.float64),
    ...
    def _generate_examples(self, source_path, split, cv_split=None):
        ...
        if split == "test":
            for filepath in path.glob('*.csv'):
                ...
                yield filepath.name, {
                    'data': self.convert_csv_to_tssi(filepath,
                                                     self.TSSI_ORDER,
                                                     # data_bounds=[0, 2.5],
                                                     scale_to=[0, 1]),
                    'label': label
            ...
            for filename in filenames:
                ...
                label = "_".join(split_by_underscore[:-2])
                yield filename, {
                    'data': self.convert_csv_to_tssi(path / filename,
                                                     self.TSSI_ORDER,
                                                     # data_bounds=[0, 2.5],
                                                     scale_to=[0, 1]),
                    'label': label
    ...
    def get_split_filenames(self, split):
        ...
        if split == "train":
            filenames = subset.loc[train_indices, "filename"]
            return filenames
        else:
            filenames = subset.loc[test_indices, "filename"]
            return filenames
